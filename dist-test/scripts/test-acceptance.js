...
async function runAcceptanceTests() {
    ...
    assert(Boolean(heroUser && heroUser.name === "Hero"), "TEST 1: Hero can create an account");
    ...
    assert(Boolean(savedHeroItem && savedHeroItem.id), "TEST 9: Hero saves the clothing item");
    ...
    const { item: wornItem } = await wardrobeService.recordWear(savedHeroItem.id, "Party", "Gala night", heroUser.id);
    ...
    assert(Boolean(heroOutfit && heroOutfit.items.includes(savedHeroItem.id)), "TEST 16: Hero creates outfit from Hero's wardrobe");
    const recommendation = RecommendationEngine.generateRecommendation(heroWardrobe, heroProfile, {
        occasion: "Party",
    });
    assert(recommendation.primary.items.some((i) => i.id === savedHeroItem.id), "TEST 17: OP AI recommendation uses Hero's actual wardrobe items");
    ...
    assert(sunWardrobeInitial.length === 0, "TEST 20 & 21: Sun logs in and does NOT see Hero's wardrobe (0 items in Sun's catalog)");
    ...
    assert(Boolean(sunSavedItem && sunSavedItem.color === "Blue"), "TEST 22: Sun adds Sun's own clothing item (Blue Shirt)");
    await authService.signOut();
    ...
    assert(heroWardrobeAfter.length === 1 && heroWardrobeAfter[0].id === savedHeroItem.id, "TEST 25: Hero still sees Hero's own Red Dress");
    assert(!heroWardrobeAfter.some((i) => i.id === sunSavedItem.id), "TEST 26: Hero does NOT see Sun's Blue Shirt (Zero Data Leakage)");
    ...
    assert(Boolean(reloadedHero && reloadedHero.id === heroUser.id), "TEST 27: Refresh / Session persists without data loss");
    ...
    assert(Boolean(manualItem && manualItem.id), "TEST 28: AI failure / offline mode does not prevent manual creation");
