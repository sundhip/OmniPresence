import { INITIAL_USER, INITIAL_WARDROBE_ITEMS, INITIAL_WEAR_EVENTS, INITIAL_OUTFITS, } from "./seedData";
...
export function notifyDataChange(entity, userId) {
    if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent(DATA_CHANGE_EVENT, { detail: { entity, userId } }));
    }
}
export class AppStorage {
    ...
    static saveWardrobe(userId, items) {
        if (!this.isClient() || !userId)
            return;
        try {
    ...
    static saveWearEvents(userId, events) {
        if (!this.isClient() || !userId)
            return;
        try {
    ...
    static saveOutfits(userId, outfits) {
        if (!this.isClient() || !userId)
            return;
        try {
    ...
    static resetUserToDefault(userId) {
        if (!this.isClient() || !userId)
            return;
        try {
            if (userId === DEMO_USER_ID) {
    ...
    static importUserData(userId, jsonString) {
        if (!this.isClient() || !userId)
            return false;
        try {
            const data = JSON.parse(jsonString);
            if (data.user)
                this.saveUser(userId, { ...data.user, id: userId });
            if (Array.isArray(data.wardrobe)) {
                this.saveWardrobe(userId, data.wardrobe.map((item) => ({ ...item, userId })));
            }
            if (Array.isArray(data.wearEvents))
                this.saveWearEvents(userId, data.wearEvents);
            if (Array.isArray(data.outfits)) {
                this.saveOutfits(userId, data.outfits.map((outfit) => ({ ...outfit, userId })));
            }
            notifyDataChange("all", userId);
            return true;
        }
