...
export const wardrobeService = {
    ...
    getItems: async (filters, userId) => {
        ...
        if (filters.colors && filters.colors.length > 0) {
            ...
            items = items.filter((item) => selectedColors.includes(item.color.toLowerCase()) ||
                item.secondaryColors?.some((sc) => selectedColors.includes(sc.toLowerCase())));
        }
        // Filter by Seasons
        if (filters.seasons && filters.seasons.length > 0) {
            items = items.filter((item) => item.season.some((s) => filters.seasons?.includes(s) || s === "All-Season"));
        }
        // Filter by Occasions
        if (filters.occasions && filters.occasions.length > 0) {
            items = items.filter((item) => item.occasion.some((occ) => filters.occasions?.includes(occ)));
        }
        // Filter by Favorite
    ...
    updateItem: async (id, updates, userId) => {
        ...
        if (index === -1)
            throw new Error("Wardrobe item not found in user catalog");
        const updated = {
            ...items[index],
            ...updates,
            userId: uid,
    ...
    recordWear: async (id, occasion = "Everyday", notes, userId) => {
        const uid = resolveUserId(userId);
        const item = await wardrobeService.getItemById(id, uid);
        if (!item)
            throw new Error("Item not found");
        const todayIso = new Date().toISOString();
        const updatedItem = await wardrobeService.updateItem(id, {
    ...
    getItemWearHistory: async (id, userId) => {
        const uid = resolveUserId(userId);
        const events = AppStorage.getWearEvents(uid);
