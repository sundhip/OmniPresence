...
export const outfitService = {
  ...
  getOutfitById: async (id: string, userId?: string): Promise<Outfit | null> => {
    const uid = resolveUserId(userId);
  ...
  getPopulatedOutfit: async (id: string, userId?: string): Promise<PopulatedOutfit | null> => {
    const uid = resolveUserId(userId);
  ...
  getAllPopulatedOutfits: async (userId?: string): Promise<PopulatedOutfit[]> => {
    const uid = resolveUserId(userId);
  ...
  updateOutfit: async (
    ...
    if (index === -1) throw new Error("Outfit not found in user plan");
  ...
  // Delete outfit
  deleteOutfit: async (id: string, userId?: string): Promise<boolean> => {
    const uid = resolveUserId(userId);
  ...
  wearOutfit: async (id: string, userId?: string): Promise<Outfit> => {
    const uid = resolveUserId(userId);
    const outfit = await outfitService.getOutfitById(id, uid);
    ...
    for (const itemId of outfit.items) {
      try {
        await wardrobeService.recordWear(
          itemId,
          outfit.occasion,
          `Worn as part of outfit: ${outfit.name}`,
          uid
        );
      } catch (e) {
